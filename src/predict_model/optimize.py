...
Kullanım:
    python optimize.py                          # mevcut veriyle
    python optimize.py --data baska_veri.xlsx  # farklı veriyle
    python optimize.py --trials 50 --timeout 300
...
def _search_space(n_rows: int) -> dict:
    ...
    """
    if n_rows < 5_000:
        return dict(iter=(200, 600, 100), depth=(3, 5), lr=(0.02, 0.15), l2=(2.0, 10.0))
    if n_rows < 30_000:
        return dict(iter=(400, 1200, 100), depth=(4, 6), lr=(0.01, 0.10), l2=(1.0, 8.0))
    if n_rows < 100_000:
        return dict(iter=(600, 1500, 100), depth=(5, 7), lr=(0.005, 0.08), l2=(0.5, 6.0))
    if n_rows < 500_000:
        return dict(iter=(800, 2000, 100), depth=(6, 8), lr=(0.003, 0.05), l2=(0.3, 4.0))
    return dict(iter=(1000, 3000, 200), depth=(7, 9), lr=(0.001, 0.03), l2=(0.1, 3.0))
...
def optimize(
    ...
    # Walk-forward split: son %20 validation
    split_idx  = int(len(feat_df) * 0.80)
    train_df   = feat_df.iloc[:split_idx]
    val_df     = feat_df.iloc[split_idx:]

    drop_cols    = [TARGET_COL, DATE_COL, GROUP_COL, KAYNAK_COL, VARIS_COL]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trials",  type=int, default=30,      help="Optuna trial sayısı")
    parser.add_argument("--timeout", type=int, default=180,     help="Timeout (saniye)")
    args = parser.parse_args()

    result = optimize(args.data, n_trials=args.trials, timeout=args.timeout)
